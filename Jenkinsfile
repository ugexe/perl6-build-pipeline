                            ...
                            withEnv(['PERL_TEST_HARNESS_DUMP_TAP=$TEST_DUMP_DIR/nqp']) {
                                timeout(time: 5, unit: 'MINUTES') {
                                    sh(returnStatus: true, script: '''
                                        ...
                                        prove --formatter TAP::Formatter::JUnitREGRU -r -j4 -e ./nqp t/nqp t/moar t/hll t/qast t/qregex t/serialization
                                    ''')
                                }
                                junit "**/*.xml"
                            }
                        ...
                        dir('rakudo') {
                            ...
                            withEnv(['PERL_TEST_HARNESS_DUMP_TAP=$TEST_DUMP_DIR/rakudo']) {
                                timeout(time: 20, unit: 'MINUTES') {
                                    sh(returnStatus: true, script: '''
                                        mkdir -p "$PERL_TEST_HARNESS_DUMP_TAP"
                                        prove --formatter TAP::Formatter::JUnitREGRU -r -j4 -e './perl6 -Ilib' t/
                                    ''')
                                }
                                junit "**/*.xml"
                            }
